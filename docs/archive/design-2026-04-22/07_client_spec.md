...
1. **WRONG_NODE 的捕捉**: 若 K8s Node Pod 掛掉、或新 Node 加入，etcd lease 失效期間 (5s 內) 會有不一致的空窗期。此段期間，Client 算出的 IP 可能錯誤，會收到 Server 退回的 `ErrorCode_WRONG_NODE`。
2. **Backoff & Retry 機制**（可選、尚未於 `cmd/client` 全面實作）: 針對 `WRONG_NODE`，應在拓樸收斂後**重新**呼叫 `GetNodeIP`（新表仍由同套件 resolver 內之 `BuildRoutingTable`+`Store` 定義）再送，而非在 Client 內自算 slot。
...
2. **DISCOVERY_ERROR / WRONG_NODE**：前者為 resolver 尚未就緒或無 owner；後者為已送達節點但所有權已變，需刷新拓樸後再 `GetNodeIP`，**不可**在 client 內以本地公式「修正」目標而繞開 **`pkg/discovery` 內**之槽位/表實作。
