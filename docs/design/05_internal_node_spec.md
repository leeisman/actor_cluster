...
2. **所有權防護（Ownership Guard）**：呼叫 `discovery.TopologyResolver.GetNodeIP`，確認此 Envelope 應屬於本節點，若非 owner 立刻回傳 `ERR_WRONG_NODE`（見 `04_remote_spec.md` §7）並驅逐本地殘留 Actor。
...
5. **結果聚合**：`aggregatorLoop` 依時間（5ms）或數量（1000）門檻批次送回 Gateway。多 stream 並發時，Node 以 `(tenant_id, uid, request_id)` 查 reply route；其中 `request_id` 由上游保證為全域唯一，因此可安全定位原始 `BatchSender`。
...
`RemoteResult.error_code` 與 gRPC transport 之穩定字串以 **`pkg/remote/errors.go`** 與 **`04_remote_spec.md` §7** 為單一真相；多 gRPC stream 併發時之回包路由見下節 `replyRoutes` / `aggregatorLoop`。
...
3. background goroutine:
   ...
   → Drain()                          （抽取信箱殘留訊息）
   → Emit `ERR_WRONG_NODE` × N       （Gateway 毫秒級收到錯誤，即時 Retry）
```
...
1. **多 gRPC stream 併發時不可共用單一 `BatchSender` 欄位**：以 `(tenant_id, uid, request_id)` 在 `Send` 到 Actor 前註冊、Emit 時取回，讓 `aggregatorLoop` 依 stream 分組回傳；歷史曾誤用「最後一條 stream 覆寫 `sender`」導致高併發多 client 時回包錯流。
